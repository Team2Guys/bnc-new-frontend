  ...
  return (
    <>
      <div className="flex flex-col justify-center items-center space-y-4 px-2">
        ...
        <p
          className="font-normal text-xs sm:text-base leading-6 sm:leading-9 text-center text-[#666768]  border-b-[#A9B4A4] pb-2 "
        ...
        {isLoading
          ...
          : filteredProducts &&
            filteredProducts.map((arr: IProduct, index: number) => {
              let product_Images = arr?.imageUrls?.find(
                (value) =>
                  value?.name?.trim()?.toLowerCase() ===
                  subCategory?.trim()?.toLowerCase(),
              );
              let findModel = arr?.modelDetails?.find(
                (value) =>
                  value?.name?.trim()?.toLowerCase() ===
                  subCategory?.trim()?.toLowerCase(),
              );
