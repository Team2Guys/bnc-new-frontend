...
const ProductDetail = ({ title, filterProduct }: IProductDetail) => {
  ...
  const [processedTabDataDetail, setProcessedTabDataDetail] =
    useState<TabDataItem[]>(tabDataDetail);
  ...
  const isCurtainsCategory = title
    ?.toLowerCase()
    .includes('motorised curtains');

  useEffect(() => {
    if (!isCurtainsCategory) return;
    const replaceBlindsWithCurtains = (text: string) =>
      text.replace(/blinds/gi, 'curtains');
    const processedData = tabDataDetail.map(
      ({ title, heading, description, ...rest }) => ({
  ...
  return (
    <div>
      <Breadcrumb
        slug={filterProduct.category.breakcrum}
        title={title}
        categorylink={filterProduct.category.categoryCustomUrl}
      />
      <Container className="grid grid-cols-12 mt-5 md:mt-10 gap-4 xl:gap-8 max-sm:px-0">
        <div className="col-span-12 md:col-span-6 xl:col-span-5 px-2">
          <Thumbnail
            images={filterProduct.imageUrls}
            selectedColor={colorImage}
            setColorImage={setColorImage}
            videos={filterProduct.videos}
      ...
      {isMotorisedCategory && (
        <InfoTabs
          ...
          isCurtainsCategory={isCurtainsCategory}
          isMotorisedCategory={isMotorisedCategory}
        />
      )}
      <div className="grid grid-cols-12 w-full gap-y-8">
        ...
        >
          <div ref={ref}>
            {show ? (
              <VideoGuide
                videos={isMotorisedCategory ? '' : filterProduct.videos}
                isMotorisedCategory={isMotorisedCategory}
              />
            ) : (
      ...
      <Information
        privacySectoin={filterProduct.privacySectoin}
        privarcyImage={
          filterProduct?.privarcyImage || filterProduct.imageUrls[0]
        }
      />
    </div>
  );
