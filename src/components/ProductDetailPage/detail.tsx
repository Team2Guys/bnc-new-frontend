...
const Detail = ({ data }: DetailProps) => {
  ...
  return (
    <div className=" space-y-2 sm:space-y-4 max-w-[650px]">
      <h1 className="font-futura font-bold text-2xl xl:text-5xl text-primary px-2">
        {data.title}
      </h1>

      <div className="flex flex-wrap items-center gap-2 lg:gap-4 px-2 !mb-4">
        {Array.isArray(data?.topImages) &&
          data.topImages.map((feature, index) => (
            <div
              key={index}
              className="rounded-full py-2 px-4 flex items-center gap-1 bg-[#F2F2F2]"
            >
              <Image
      ...
      <p className="px-2" suppressHydrationWarning>
        <span
          className="font-roboto blog-content"
      ...
      {data.colorsImages && data.colorsImages.length > 0 && (
        <>
          ...
          <div className=" flex items-start gap-2 md:pb-10 px-2">
            ...
            <p className="border rounded-lg font-roboto h-9 md:h-12 flex items-center px-2 text-xs md:text-base max-w-[140px] xs:max-w-full">
              You can still pick from 3000+ fabric colours
            </p>
          </div>
        </>
      )}
      ...
      <Link
        href="/request-appointment/"
        className="bg-secondary hover:opacity-65 text-primary py-3 px-6 font-semibold hidden md:block rounded-md w-full sm:w-fit font-roboto text-center "
      >
        Book A Free Visit
      </Link>
      ...
      <div className="flex max-sm:flex-col sm:items-stretch sm:gap-2 sm:pt-5 px-2">
        {featuresinfo.map((feature, index) => (
          <div
            key={index}
            className="sm:border sm:rounded-sm flex sm:flex-col gap-1 justify-center items-center sm:space-y-2 py-2 sm:px-4 sm:min-h-[140px] w-fit"
          >
            <Image
              src={feature.icon}
              height={200}
              ...
              alt="feature"
            />
            <p className="font-roboto sm:max-w-32 sm:text-center text-base sm:text-sm">
              {feature.text}
            </p>
          </div>
        ))}
