...
export default function InfoTabs({
  ...
  const activeTabData = useMemo<TabDataItem[]>(
    () => tabData ?? Data,
    [tabData],
  );
    ...
    <div className={`bg-secondary-foreground ${isHome ? '' : 'mt-10'}`}>
      <Container>
        <div className="text-center text-primary font-bold font-futura text-2xl lg:text-4xl py-5 sm:py-7">
          {isHome ? (
            <>
              <span className="text-secondary">Trusted</span> by{' '}
              <span className="text-secondary">18,000+ </span>Happy Customers
            </>
          ) : (
            <span>
              Standout Features of Smart{' '}
              {isCurtainsCategory ? 'Curtains' : 'Blinds'}
            </span>
          )}
        </div>
        <div className="hidden sm:flex justify-between text-center">
          {activeTabData.map((tab, index) => (
            ...
            >
              ...
              />
              <p
                className={`text-primary text-wrap lg:px-6 :px-0 ${isHome ? 'text-lg xl:text-xl 2xl:px-16' : '2xl:px-10 text-base xl:text-xl'}`}
              >
                {tab.title}
              </p>
            </button>
          ...
          <div className="grid md:grid-cols-2 gap-6 lg:gap-16 2xl:gap-24 items-center md:py-8 p-2">
            <div className="flex flex-col justify-center items-start space-y-5 lg:space-y-12">
              ...
              <Link
                ...
                className="border bg-secondary text-primary font-semibold text-base font-roboto px-6 py-2 md:py-4 rounded-xl hover:bg-primary-dark transition hover:opacity-65"
              >
                Book A Free Visit
              </Link>
            </div>
            <div
              className={`relative w-full object-contain ${isHome ? 'px-4 h-[235px] lg:h-[454px]' : 'h-[500px] lg:h-[547px]'}`}
            >
              <Image
              ...
              {isHome && (
                <div className="absolute bottom-10 -left-12 lg:bottom-32 xl:bottom-[73px] lg:-left-[80px] 2xl:-left-24 w-[116px] h-[56px] md:h-[70px] md:w-[140px] lg:h-[100px] lg:w-[200px] bg-primary flex flex-col items-center justify-center -rotate-90">
                  <span className="text-white font-semibold text-sm md:text-[22px] lg:text-2xl font-futura">
                    20 Years
                  </span>
                  <span className="text-white text-[8px] md:text-[10px] lg:text-sm font-medium md:mt-2 font-roboto">
        ...
        <div className="md:hidden">
          <div
            className={`relative mb-4 w-full ${isHome ? 'h-[235px]' : 'h-[450px]'}`}
          >
            <Image
            ...
            {isHome && (
              <div className="absolute bottom-[30px] -right-[30px] w-[116px] h-[56px] bg-primary flex flex-col items-center justify-center -rotate-90">
                <span className="text-white font-semibold text-sm font-futura">
                  20 Years
                </span>
                <span className="text-white text-[8px] font-medium mt-1 font-roboto">
                  Making Blinds & Curtains
                </span>
              </div>
            )}
              ...
              >
                <div className="flex justify-start items-center gap-2">
                  <Image
                    ...
                    className="w-9 h-9"
                  />
                  <span className="text-primary font-roboto font-medium md:font-normal text-base leading-[130%]">
                    {tab.title}
                  </span>
                </div>
                <div>
        ...
        <div className="flex justify-center items-center">
          <Link
            href="/request-appointment/"
            className="sm:hidden my-3 inline-block bg-secondary text-primary font-medium text-lg sm:text-sm px-5 py-2 rounded-md shadow hover:bg-primary-dark transition"
