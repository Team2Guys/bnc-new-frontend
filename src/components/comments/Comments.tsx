...
function Comments({ data }: CommentsProps) {
  ...
  const comments =
    data?.comments?.filter((comment: any) => comment.status === 'APPROVED') ||
    [];
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (validateForm()) {
      ...
      try {
        setloading(true);
        await axios.post(
          `${process.env.NEXT_PUBLIC_BASE_URL}/api/blogs/${endpoint}/${id}`,
          formData,
        );
        if (isModalOpen) {
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="text-lg font-normal text-paralight flex items-center gap-2">
          ...
          <span className="flex items-center gap-1">
            ...
            <PinterestShareButton
              url={shareURL}
              media={data?.posterImage?.imageUrl}
            >
        ...
        <div className="border-t border-[#EAEAEA] mt-5">
          {currentComments &&
            ...
            currentComments.map((item: any) => (
              <div
                key={item.id}
                className="leading-8 border-b border-[#EAEAEA] my-4"
              >
                <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  onClick={() => handleReplyClick(item.id, 'parent')}
                >
                ...
                {item?.replies.map((nestedItem: any) => (
                  <div key={nestedItem.id} className="mt-4 leading-8 ps-6">
                    ...
                    <button
                      className="flex items-center gap-1"
                      onClick={() => handleReplyClick(item.id, 'nested')}
                    >
                      <BsReply className="text-[#CDB7AA]" size={18} />
                      <span className="font-medium text-base">Reply</span>
                    </button>
