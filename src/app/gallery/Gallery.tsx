...
const Gallery = ({ products }: { products: IProduct[] }) => {
  ...
  const sortedCategories = ["All", ...sortOrder.filter((cat) => uniqueCategories.includes(cat))];
  const [selectedCategory, setSelectedCategory] = useState<string>(sortedCategories[0] || "All");
  ...
  return (
    <Container className="my-10 space-y-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 xs:mt-20 mt-5 md:px-4">
        {visibleProducts && visibleProducts.map((product: IProduct, index: number) => (
          <div
            key={index}
          ...
          >
            <div className="relative block w-full h-[140px] xs:h-[300px] cursor-zoom-in" onClick={() => {
              setSelectedImage({
         ...
         <CustomModal open={openModal} onClose={() => setOpenModal(false)} title={" "} width={"max-w-xl"} isheader>
