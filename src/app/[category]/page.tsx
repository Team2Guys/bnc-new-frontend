...
const Products = async ({ params }: Props) => {
  ...
  const matchingLink: any = links.find((link) =>
    slug.includes(link.href.replace(/^\//, '')),
  );

  const uniqueProducts = Array.from(
    new Map(
      [...category.products, ...category.recalledProducts].map((product) => [
        product.id,
        product,
      ]),
    ).values(),
  ...
  const filteredProducts = uniqueProducts
    ?.filter((product: IProduct) => product.status === 'PUBLISHED')
    ?.sort((a: IProduct, b: IProduct) => {
      const orderArray = subcategoryMap[category.title?.toLowerCase()];
      if (!orderArray) return 0;
      const indexA = orderArray.findIndex(
        (item) => item.toLowerCase().trim() === a.title?.toLowerCase().trim(),
      );
      const indexB = orderArray.findIndex(
        (item) => item.toLowerCase().trim() === b.title?.toLowerCase().trim(),
      );
      return (
        (indexA === -1 ? orderArray.length : indexA) -
        (indexB === -1 ? orderArray.length : indexB)
      );
    });
  ...
  return (
    <>
      ...
      <Product categories={category} filteredItems={filteredProducts} />
    </>
  );
};
