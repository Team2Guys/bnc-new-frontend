...
export default function ViewReviews({
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: IREVIEWS) => (
        <LiaEdit
          className={`${canEditReiview ? 'cursor-pointer' : ''} ${
            !canEditReiview ? 'cursor-not-allowed text-slate-200' : ''
          }`}
          size={20}
          onClick={() => {
            if (canEditReiview) {
    ...
    },
    {
      ...
      render: (record: IREVIEWS) =>
        loading == record.id ? (
          ...
          <RiDeleteBin6Line
            className={`${canDeleteReiview ? 'text-red-600 cursor-pointer' : ''} ${
              !canDeleteReiview ? 'cursor-not-allowed text-slate-200' : ''
            }`}
            size={20}
            onClick={() => {
